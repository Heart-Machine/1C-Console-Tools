...
Процедура КомандаСохранитьВыборкуВФайл(Команда)
	Оповещение = Новый ОписаниеОповещения("КомандаСохранитьВыборкуВФайлЗавершение", ЭтаФорма);
	ПоказатьВопрос(Оповещение, "Сохранить строки выборки в файл?", РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет,
		"Сохранение выборки");
КонецПроцедуры

&НаКлиенте
Процедура КомандаСохранитьВыборкуВФайлЗавершение(Результат, ДопПараметры) Экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		Если Не ЗначениеЗаполнено(ВыгрузкаВФайлПутьКФайлу) Или РазделительКолонок = "" Тогда
			Сообщить("Не все требуемые настройки заполнены");
			Возврат;
...
Процедура ОбновитьТаблицуНаФорме(РезультатВыполненияЗапроса, ЧислоСтрок)
	...
	Если МаксимальноеЧислоСтрокВыводимыхНаФорму = 0 Или ЧислоСтрокВыборки <= МаксимальноеЧислоСтрокВыводимыхНаФорму Тогда
		ЗначениеВРеквизитФормы(ТЗВыборка, Строка("ТЗ"));
	Иначе
