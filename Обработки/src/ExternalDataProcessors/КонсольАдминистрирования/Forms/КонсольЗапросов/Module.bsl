...
Процедура КомандаВыполнитьЗапрос(Команда)
	...
	Если НЕ ПустаяСтрока(ТекстТекущегоЗапроса) Тогда
		...
		Если ЗапросВыполненУспешно Тогда
			...
			Элементы.ГруппаТекстЗапроса.Заголовок = "Текст запроса (время выполнения: " + ПолучитьПериодСтрокой(ВремяВыполнения) + ", число строк: " + ЧислоСтрок + ")";
...
&НаКлиенте
Процедура ОчиститьСтраницу(Команда)
...
Процедура КомандаРазвернутьВыборкуДанныхЗапросаНаВсюФорму(Команда)
	...
	УстановитьПометкуЭлементаКоманда("КомандаВыборкаДанныхЗапросаНаВсюФорму", Не Элементы.КомандаВыборкаДанныхЗапросаНаВсюФорму.Пометка);
...
Процедура КомандаОчиститьВыборку(Команда)
	...
	Если Вопрос("Очистить выборку?", РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет, "Очистка выборки") = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;
...
Процедура КомандаВыполнитьИСохранитьВФайл(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ВыполнитьИСохранитьВФайлЗавершение", ЭтаФорма), "Выполнить запрос и сохранить строки в файл?", РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет, "Выгрузка результата запроса");
...
Процедура КомандаСохранитьВыборкуВФайл(Команда)
	...
	Если Вопрос("Сохранить строки выборки в файл?", РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет, "Сохранение выборки") = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьВидимостьДоступностьЭлементовФормы();
	...
	Элементы.ГрВыборкаДанныхЗапроса.Видимость 		= Не Элементы.КомандаТекстЗапросаНаВсюФорму.Пометка Или Элементы.КомандаВыборкаДанныхЗапросаНаВсюФорму.Пометка;
...
Процедура ВыполнитьИСохранитьВФайлНаСервере()
	...
	ТЗВыборка = ?(ПакетныйЗапрос, РезультатВыполненияЗапроса[НомерЗапросаВПакете].Выгрузить(), РезультатВыполненияЗапроса.Выгрузить());
	КоличествоКолонок = ТЗВыборка.Колонки.Количество();
	
	Сообщить(Формат(ТекущаяДата(),"ДФ='yyyy.MM.dd HH.mm.ss'") + ": выгрузка начата");
	ЗаписьТекста = Новый ЗаписьТекста(ВыгрузкаВФайлПутьКФайлу, ?(ПустаяСтрока(ВыгрузкаВФайлКодировкаФайла), КодировкаТекста.UTF16, ВыгрузкаВФайлКодировкаФайла));
...
Процедура СохранитьСтрокуТЗВФайл(СтрокаТЗ, КолонкиТЗ, ЗаписьТекста)
	...
	Для НомерТекущейКолонки = 0 По КоличествоКолонок - 1 Цикл
		...
		Иначе
			...
			ЗначениеЯчейки = СтрЗаменить(ЗначениеЯчейки,РазделительКолонок,ВыгрузкаВФайлЗаменятьСимволРазделителяКолонокНа);
			
			Если ВыгрузкаВФайлУдалятьСпецСимволы И НомерТекущейКолонки = ВыгрузкаВФайлНомерКолонкиДляУдаленияСпецсимволов Тогда
		...
		Если (ВыгрузкаВФайлВыводитьРазделительПослеПоследнейКолонки И НомерТекущейКолонки = КоличествоКолонок - 1) Или НомерТекущейКолонки <> КоличествоКолонок - 1 Тогда
			ЗаписьТекста.Записать(РазделительКолонок);
		КонецЕсли;
...
Процедура ЗаполнитьНастройки()
	...
	ВыгрузкаВФайлУдалятьНезначащиеСимволыВНачалеИКонцеВСтроковыхРеквизитахвыхРеквизитах = Истина;
...
Процедура СоздатьНовыйЗапрос(ТекстНовогоЗапроса = "", НаименованиеНовогоЗапроса = "")
	...
	НоваяСтрокаСпискаЗапросов.ИмяЗапроса 	= ?(ПустаяСтрока(НаименованиеНовогоЗапроса), "Запрос " + НомерЗапроса, НаименованиеНовогоЗапроса);
...
Процедура СоздатьНовыйЗапросНаСервере(НомерЗапроса)
	...
	ЭлементГруппа = Элементы.Добавить("ГруппаЗапрос" + НомерЗапроса, Тип("ГруппаФормы"), Элементы.ГруппаСписокЗапросовСтраницы);
	...
	Иначе
		Элементы.Переместить(ЭлементГруппа, Элементы.ГруппаСписокЗапросовСтраницы, Элементы["ГруппаЗапрос" + НомерСледующегоЗапроса]);
	КонецЕсли;
	
	//Элемент, который необходимо сделать видимым после активизации другой страницы
	ЭлементДекорация = Элементы.Добавить("ГруппаЗапрос" + НомерЗапроса + "Заглушка", Тип("ДекорацияФормы"), ЭлементГруппа);
...
Процедура ОтобразитьЭлементыФормыНаУказаннойВкладке(ИмяГруппы)
	...
	Если СтрокаТЗ.ВремяВыполнения >= 0 Тогда
		Элементы.ГруппаТекстЗапроса.Заголовок = "Текст запроса (время выполнения: " + ПолучитьПериодСтрокой(СтрокаТЗ.ВремяВыполнения) + ", число строк: " + СтрокаТЗ.ЧислоСтрок  + ")";
	Иначе 
		Элементы.ГруппаТекстЗапроса.Заголовок = "Текст запроса";
	КонецЕсли;
...
Функция ПолучитьТекстТекущегоЗапроса()
	...
	РабочийТекстЗапроса = ?(ПустаяСтрока(Элементы.ТекстЗапроса.ВыделенныйТекст),ТекстЗапроса,Элементы.ТекстЗапроса.ВыделенныйТекст);
...
Функция ПодставитьТекстыВложенныхЗапросов(ИсходныйТекст)
	...
	Иначе
		НомерЗапроса = СокрЛП(Сред(ПраваяЧастьСНомеромВложенногоЗапроса, 1, НомерСимволаПослеНомераВложенногоЗапроса - 2));
	КонецЕсли;
...
Процедура ДобавитьСимволыКомментария() 
	...
	Для НомерСтроки = 2 По ПоследняяСтрока Цикл
		...
		НомерСимволаПСВТекстеЗапроса = 
			НомерСимволаПСВТекстеЗапроса + НомерСимволаПСВОбрабатываемойСтроке;
			
		Если НомерСтроки >= ПерваяСтрока Тогда
			ТекстЗапроса = 		
				Сред(ТекстЗапроса, 1, НомерСимволаПСВТекстеЗапроса)
				+ "//"
				+ Сред(ТекстЗапроса, НомерСимволаПСВТекстеЗапроса + 1);
		КонецЕсли;
	КонецЦикла; 
...
Процедура УдалитьСимволыКомментария()
	Перем ПерваяСтрока, ПерваяКолонка, ПоследняяСтрока, ПоследняяКолонка,
		НомерПоследнегоСимволаВОбрабатываемойСтроке, НомерПервогоСимволаСтрокиВТекстеЗапроса, НомерПоследнегоСимволаСтрокиВТекстеЗапроса;
	...
	Для НомерСтроки = 1 По ПоследняяСтрока Цикл
		...
		НомерПоследнегоСимволаСтрокиВТекстеЗапроса = 
			НомерПоследнегоСимволаСтрокиВТекстеЗапроса + НомерПоследнегоСимволаВОбрабатываемойСтроке;
			
		Если НомерСтроки >= ПерваяСтрока Тогда
			//Если это последняя строка, т.е. не найдено символов ПС, вызываем функцию Сред без указания длины строки
			ИсходнаяСтрокаСКомментариями = ?(НомерПоследнегоСимволаВОбрабатываемойСтроке = 0,
			    Сред(ТекстЗапроса, НомерПервогоСимволаСтрокиВТекстеЗапроса),
				Сред(ТекстЗапроса, НомерПервогоСимволаСтрокиВТекстеЗапроса, НомерПоследнегоСимволаСтрокиВТекстеЗапроса - НомерПервогоСимволаСтрокиВТекстеЗапроса));
			...
			Если ИсходнаяСтрокаСКомментариями <> ИзмененнаяСтрокаСКомментариями Тогда
				ТекстЗапроса =
					Сред(ТекстЗапроса, 1, НомерПервогоСимволаСтрокиВТекстеЗапроса - 1)
					+ ИзмененнаяСтрокаСКомментариями
					+ ?(НомерПоследнегоСимволаВОбрабатываемойСтроке = 0,
						"",
						Сред(ТекстЗапроса, НомерПоследнегоСимволаСтрокиВТекстеЗапроса));
						
				//Еслит длина строки изменилась, корректируем значение переменной
				НомерПоследнегоСимволаСтрокиВТекстеЗапроса = 
					НомерПоследнегоСимволаСтрокиВТекстеЗапроса 
					+ (СтрДлина(ИзмененнаяСтрокаСКомментариями) - СтрДлина(ИсходнаяСтрокаСКомментариями));
			КонецЕсли;
		КонецЕсли;
					
		НомерПервогоСимволаСтрокиВТекстеЗапроса = 
			НомерПоследнегоСимволаСтрокиВТекстеЗапроса + 1 ;
...
Функция УдалитьСимволыКомментарияИзСтроки(Знач СтрокаСКомментариями)
	...
	Если СимволыНайдены И ВНачалеСтроки Тогда
		РезультирующаяСтрока = 		
			Сред(СтрокаСКомментариями, 1, НомерСимволовКомментария - 1)
			//если справа от символов комментария не было знаков, ничего не прибавляем и к результирующей строке
			+ ?(СтрДлина(СтрокаСКомментариями) = (НомерСимволовКомментария + 1), "", Сред(СтрокаСКомментариями, НомерСимволовКомментария + 2));
	Иначе
...
Процедура ОбновитьТаблицуНаФорме(РезультатВыполненияЗапроса, ЧислоСтрок)
	...
	Для НомерТекущейКолонки = 0 По КоличествоКолонок - 1 Цикл
		...
		МассивДобавляемыхРеквизитов.Добавить(
			Новый РеквизитФормы(
				ИмяКолонки, 				
				Новый ОписаниеТипов(ТЗВыборка.Колонки[НомерТекущейКолонки].ТипЗначения,,Массив),
				"ТЗ"));		
	...
	Иначе
		...
		Для Каждого СтрокаВыборкиТЗ Из ТЗВыборка Цикл
			...
			Если СчетчикСтрок = МаксимальноеЧислоСтрокВыводимыхНаФорму Тогда
				Сообщить("На форму выведено первых " + МаксимальноеЧислоСтрокВыводимыхНаФорму + " строк из " + ЧислоСтрокВыборки);
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура СохранитьТекстТекущегоЗапроса()
	...
	СтрокаТЗ = СписокЗапросов.НайтиСтроки(Новый Структура("НомерЗапроса", Число(Сред(Элементы.ГруппаСписокЗапросовСтраницы.ТекущаяСтраница.Имя,13))))[0];
	СтрокаТЗ.ТекстЗапроса = ТекстЗапроса;
...
Функция ПолучитьПериодСтрокой(ЧислоСекундВсего)
	...
	Иначе
		...
		ПериодСтрокой 
			= ?(ЧислоЧасов = 0, "", Строка(ЧислоЧасов) + " ч. ")
				+ ?(ЧислоМинут = 0, "", Строка(ЧислоМинут) + " мин. ") 
				+ ЧислоСекунд + " сек.";
	КонецЕсли;
...
Процедура ОбновитьПредставлениеДатыПоВыбранномуФормату()
	...
	Элементы.ДекорацияФорматДаты.Заголовок = Формат(Дата(1983,04,18,13,14,15), "ДФ='" + ВыгрузкаВФайлФорматРеквизитовТипаДата + "'");
...
Процедура СписокПараметровЗначениеПараметраНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Элементы.СписокПараметровЗначениеПараметра.ВыбиратьТип = ТипЗнч(Элементы.СписокПараметров.ТекущиеДанные.Значениепараметра)= Тип("Неопределено");	
КонецПроцедуры
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗакрытиеФормыПодтверждено Тогда
		Отказ = Истина;
		ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтаФорма),"Закрыть консоль запросов?", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
&НаКлиенте
Процедура ГруппаЗапросыМетаданныеПараметрыПриСменеСтраницы(Элемент, ТекущаяСтраница)
		
	Элементы.ФормаГруппаСимволы.Видимость = Элементы.ГруппаЗапросыМетаданныеПараметры.ТекущаяСтраница = Элементы.ГруппаТекстЗапроса;
...
Процедура ГруппаСписокЗапросовСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	ИначеЕсли Элементы.ГруппаСписокЗапросовСтраницы.ТекущаяСтраница = Элементы.ГруппаЗапросУдалить Тогда
		...
		Иначе 
			...
			Элементы.ГруппаСписокЗапросовСтраницы.ТекущаяСтраница = Элементы["ГруппаЗапрос" + СписокЗапросов[0].НомерЗапроса];
