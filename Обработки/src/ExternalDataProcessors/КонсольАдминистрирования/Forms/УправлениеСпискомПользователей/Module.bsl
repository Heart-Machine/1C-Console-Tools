...
Функция ПолучитьСвойстваПользователяСправочника(Пользователь)
	...
	Если ТипЗнч(Пользователь) = Тип("СправочникСсылка.Пользователи")
	 Или ТипЗнч(Пользователь) = Тип("СправочникОбъект.Пользователи") Тогда
...
Процедура ЗаписатьИзмененияНаСервере()
	...
	Для каждого Строка Из ТаблПользователей Цикл
	   	Если Строка.ДанныеИзменены Тогда
			НовоеОписаниеПользователяИБ = Пользователи.СвойстваПользователяИБ(Строка.Пользователь.ИдентификаторПользователяИБ);
			ЗаполнитьЗначенияСвойств(НовоеОписаниеПользователяИБ, Строка);
			Пользователи.УстановитьСвойстваПользователяИБ(Строка.Пользователь.ИдентификаторПользователяИБ, НовоеОписаниеПользователяИБ);
			Если НовоеОписаниеПользователяИБ.ПользовательИБ.ЗащитаОтОпасныхДействий.ПредупреждатьОбОпасныхДействиях <> Строка.ПредупреждатьОбОпасныхДействиях Тогда
...
Процедура ВыбратьДомен(ВыбЗнач, Параметры) Экспорт
	...
	Если ВыбЗнач <> Неопределено Тогда
		...
		Если ВременнаяТаблПользователи.Количество() > 0 Тогда
			...
			КонецЦикла;
			Для каждого Строка Из ТаблПользователей Цикл
				Если НРег(Строка(Строка.Пользователь)) <> "администратор" И (Строка.АутентификацияОС Или Строка.АутентификацияСтандартная) Тогда 
					...
					Если Найденный <> Неопределено Тогда
						НайденныйЮзер = ВременнаяТаблПользователи.НайтиПоИдентификатору(Найденный.ПолучитьИдентификатор());
						УстановитьПользователяОС(Строка, НайденныйЮзер.Значение, Домен);
					//Иначе
					//	Для каждого СтрокаП Из ВременнаяТаблПользователи Цикл
					//		ВозможныйЮзер = ПреобразоватьИмяПользователя(СтрокаП.Значение);
					//		Если ВозможныйЮзер = НашЮзер Тогда 
					//			УстановитьПользователяОС(Строка, СтрокаП.Значение, Домен);
					//			Прервать;
					//		КонецЕсли;
					//	КонецЦикла;
					КонецЕсли;
...
Процедура ТаблПользователейПередНачаломИзменения(Элемент, Отказ)
	Если Элемент.ТекущийЭлемент.Имя = "ТаблПользователейПользователь" Или Элемент.ТекущийЭлемент.Имя = "ТаблПользователейНедействителен" Тогда
		Возврат;
	КонецЕсли;	
	ТекСтрока = Элементы.ТаблПользователей.ТекущиеДанные;
	Если ТекСтрока.Недействителен Тогда
...
Процедура УстановитьПометкуУдаления(Команда)
	...
	ПоказатьВопрос(Оповещение, СтрШаблон("%1 у ""%2"" пометку на удаление?", ТекстУстановить, Пользователь),РежимДиалогаВопрос.ДаНет);
