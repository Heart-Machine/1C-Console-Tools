...
Функция ПолучитьЗаголовкиФайФайлССервера(ФайлНаВебСервере)
	...
	ЗаголовкиЗапроса = Соединение.ПолучитьЗаголовки(HTTPЗапрос);
...
&НаСервереБезКонтекста
Функция ПолучитьФайлССервера(ФайлНаВебСервере, Каталог)
	...
	Если HTTPОтвет.КодСостояния < 200 Или HTTPОтвет.КодСостояния >= 300 Тогда
		ФайлОтвета = Новый ЧтениеТекста(Каталог + "\temp.htm", КодировкаТекста.UTF8);
...
Функция ПолучитьHTTPОтвет(ПутьКФайлуНаСервере, ИмяФайла, Соединение, Заголовки, флаг = Истина)
	...
	Если флаг и HTTPОтвет.КодСостояния >= 200 и HTTPОтвет.КодСостояния <= 299 Тогда 
		флаг = Ложь;
		НазваниеФайла = HTTPОтвет.Заголовки.Получить("Content-Disposition");
		Если НазваниеФайла <> Неопределено Тогда 
			Если Найти(НазваниеФайла,"filename=""") > 0 Тогда
				ИмяФайлаВр = Сред(НазваниеФайла, 10 + Найти(НазваниеФайла,"filename="""),СтрДлина(НазваниеФайла)- 10 - Найти(НазваниеФайла,"filename="""));
			КонецЕсли;
...
Процедура ПолучитьСписокБаз()
	...
	Если ФайлСБазами.Существует() Тогда
		
		ФайлСБазамиТекст = Новый ЧтениеТекста(ИмяФайла, КодировкаТекста.UTF8);
		ТекСтрока = ФайлСБазамиТекст.ПрочитатьСтроку();
		...
		Пока НЕ ТекСтрока = Неопределено Цикл			
			...
			ИначеЕсли СтрНачинаетсяС(ТекСтрока, "Connect=") Тогда
				Подключение = Сред(ТекСтрока, 9, СтрДлина(ТекСтрока)-9);
				НоваяСтрока = СписокБаз.Добавить();
				НоваяСтрока.НазваниеБазы = НазваниеИБ;
				НоваяСтрока.Подключение = Подключение;
			КонецЕсли;
			ТекСтрока = ФайлСБазамиТекст.ПрочитатьСтроку();
		КонецЦикла;
		ФайлСБазамиТекст.Закрыть();
	КонецЕсли;
	СписокБаз.Сортировать("НазваниеБазы");
	
КонецПроцедуры // ПолучитьСписокБаз()
...
Процедура ПолучитьДанныеОЗагруженномКлассификаторе()
	...
	КлассификаторОбновлен = ЗагруженныеАдресныеСведения.Получить("ИспользоватьЗагруженные");
