...
Процедура СкачатьФИАС(Команда)
	...
	ПоказатьВопрос(Оповещение, "Скачать файл с сайта? Это займет некоторое время.", РежимДиалогаВопрос.ДаНет, , ,
		"Оповещение");
...
Процедура ПолучитьФайлССервераЗавершение(Результат, ПараметрыОповещения) Экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Для Каждого Строка Из МассивСубъектовРФ Цикл
			Состояние("Скачивание ФИАС", Счетчик / МассивСубъектовРФ.Количество() * 100, "Скачивание "
				+ Строка.Представление + " (" + (Счетчик + 1) + " из " + МассивСубъектовРФ.Количество() + ")",
				БиблиотекаКартинок.Загрузить);

			URL = ПараметрыОповещения.URL + Строка.КодСубъектаРФ;
			НастройкиПолучения = Новый Структура;
			НастройкиПолучения.Вставить("ПутьДляСохранения", ПараметрыОповещения.ПутьФайлаФИАС + "\" + Формат(
				Строка.КодСубъектаРФ, "ЧЦ=2; ЧВН=") + ".ZIP");
...
Процедура ЗагрузитьКлассификаторИзКаталога(Знач КаталогДанных)
	...
	АдресныйКлассификаторКлиент.АнализДоступностиФайловКлассификатораВКаталоге(ОписаниеОповещения, КодыРегионов,
		КаталогДанных, ПараметрыЗагрузки);
...
Процедура ЗапуститьФоновуюЗагрузкуИзКаталогаКлиентаПослеПомещенияФайлов(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если Позиция <= ПомещенныеФайлы.ВГраница() Тогда
		...
		Файл.НачатьПолучениеУниверсальногоВремениИзменения(
			Новый ОписаниеОповещения("ЗапуститьФоновуюЗагрузкуИзКаталогаКлиентаПослеПолученияВремениИзменения",
			ЭтотОбъект, ДополнительныеПараметры));
	...
	Иначе // выход из цикла
		...
		Задание = ЗапуститьФоновуюЗагрузкуНаСервере(ДополнительныеПараметры.ПараметрыЗагрузки.КодыРегионов,
			ПомещенныеФайлы, Режим, ЗагружатьИсториюИзмененийАдресныхОбъектов, УникальныйИдентификатор);
...
&НаКлиенте
Процедура ЗапуститьФоновуюЗагрузкуИзКаталогаКлиентаПослеПолученияВремениИзменения(ВремяИзменения,
	...
	ЗапуститьФоновуюЗагрузкуИзКаталогаКлиентаПослеПомещенияФайлов(ДополнительныеПараметры.ПомещенныеФайлы,
...
Процедура ПослеФоновойЗагрузкиИзКаталога(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Ошибка" Тогда
		...
		ТекстОшибки = ТекстОшибки + НСтр(
			"ru = 'Необходимо сохранить файлы с сайта «1С» http://its.1c.ru/download/fias2 на диск, а затем загрузить в программу.'")
			+ Символы.ПС;
...
Процедура ОбновитьИнтерфейсПоКоличествуЗагружаемых()
	...
	// Страница загрузки
	ТекстОписанияЗагрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = 'Загружаются данные по выбранным регионам (%1)'"), ВыбраноРегионовДляЗагрузки);
...
&НаСервереБезКонтекста
Функция ЗапуститьФоновуюЗагрузкуНаСервере(Знач КодыРегионов, Знач ОписаниеФайловЗагрузки, Знач Режим,
	...
	ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне(
		"АдресныйКлассификаторСлужебный.ФоновоеЗаданиеЗагрузкиКлассификатораАдресов", ПараметрыМетода,
		ПараметрыВыполнения);
...
Процедура ПолучитьАктуальнуюДатуФИАС()
	...
	ДоступныеДанные = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML, ФабрикаXDTO.Тип("http://www.v8.1c.ru/ssl/AddressSystem",
		"Publications"));
