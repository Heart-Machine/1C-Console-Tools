...
Процедура ПолучитьСписокБаз()
	...
	Для каждого ТекСтрока Из СтрокиФайла Цикл
		...
		ИначеЕсли СтрНачинаетсяС(ТекСтрока, "ID=") Тогда
			СписокБазБезГрупп[СписокБазБезГрупп.Количество()-1].ID = Прав(ТекСтрока, СтрДлина(ТекСтрока) - СтрНайти(ТекСтрока, "="));
			// OrderInList=	
		ИначеЕсли СтрНачинаетсяС(ТекСтрока, "OrderInList=") Тогда	
			СписокБазБезГрупп[СписокБазБезГрупп.Количество()-1].OrderInList = Прав(ТекСтрока, СтрДлина(ТекСтрока) - СтрНайти(ТекСтрока, "="));
			// Folder=	
		ИначеЕсли СтрНачинаетсяС(ТекСтрока, "Folder=") Тогда
			СписокБазБезГрупп[СписокБазБезГрупп.Количество()-1].Folder = Прав(ТекСтрока, СтрДлина(ТекСтрока) - СтрНайти(ТекСтрока, "=/") - 1);
			// OrderInTree=	
		ИначеЕсли СтрНачинаетсяС(ТекСтрока, "OrderInTree=") Тогда
			СписокБазБезГрупп[СписокБазБезГрупп.Количество()-1].OrderInTree = Прав(ТекСтрока, СтрДлина(ТекСтрока) - СтрНайти(ТекСтрока, "="));
		КонецЕсли;
	КонецЦикла;
