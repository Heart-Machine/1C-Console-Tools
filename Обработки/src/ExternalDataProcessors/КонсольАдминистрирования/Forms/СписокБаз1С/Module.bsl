...
Функция УстановкаВерсииКонфигурации(Строка)
	...
	Если ComConnector = Неопределено Тогда
...
Процедура ВключитьОтображениеНовостейНаСервере()
	...
	Для Каждого ВыделеннаяСтрока Из Элементы.СписокБаз.ВыделенныеСтроки Цикл
		...
		Если Строка.ЭтоПапка Тогда
			ПройтисьПоДереву(Строка, "УправлениеНовостямиВключить");
		Иначе
			УправлениеНовостямиВключить(Строка);
		КонецЕсли;
	КонецЦикла;
	ЗначениеВРеквизитФормы(Дерево, "СписокБаз");
КонецПроцедуры
...
&НаСервере
Функция УправлениеНовостямиВключить(Строка)
...
&НаСервере
Функция УправлениеНовостямиВыключить(Строка)
...
Процедура УбратьОтображениеНовостейНаСервере()
	...
	Для Каждого ВыделеннаяСтрока Из Элементы.СписокБаз.ВыделенныеСтроки Цикл
		...
		Если Строка.ЭтоПапка Тогда
			ПройтисьПоДереву(Строка, "УправлениеНовостямиВыключить");
		Иначе
			УправлениеНовостямиВыключить(Строка);
		КонецЕсли;
	КонецЦикла;
	ЗначениеВРеквизитФормы(Дерево, "СписокБаз");
КонецПроцедуры
...
&НаКлиентеНаСервереБезКонтекста
Функция ComConnector()
	Попытка
		ComConnector = Новый COMОбъект("V83.COMConnector");
		Возврат ComConnector;
	Исключение
#Если НаКлиенте Тогда
		Сообщить("На клиентском компьютере не зарегистрирована компонента ""V83.COMConnector""");
#Иначе
			Если ЭтоФайловаяБаза() Тогда
				Сообщить("На клиентском компьютере не зарегистрирована компонента ""V83.COMConnector""");
			Иначе
				Сообщить("На сервере не зарегистрирована компонента ""V83.COMConnector""");
			КонецЕсли;
#КонецЕсли
		Возврат Неопределено;
	КонецПопытки
...
Функция БлокировкаРегламентныхЗаданий(УстановитьБлокировку = Неопределено) Экспорт
	...
	КонецЕсли;
	Попытка
		СоединениеСАгентомСервера = ComConnector.ConnectAgent(СтруктураСоединения.Srvr);
	Исключение
	...
	Для Каждого РабочийПроцесс Из МассивРабочихПроцессов Цикл
		СоединениеСРабочимПроцессом = ComConnector.ConnectWorkingProcess("tcp://" + РабочийПроцесс.HostName + ":"
